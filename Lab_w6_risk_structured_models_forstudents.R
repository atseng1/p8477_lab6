...
########################
# [code to look at model outputs on log-scale]
...
## only look at the number of infectious
...
beta=   # FILL IN YOUR BETA MATRIX HERE
...
## or directly:
R0=eigen(n.matrix %*% beta)$values[1]/gamma # %*% is matrix multiplication
...
########################
# [code to compute all R0s for all alphas together using a big for-loop]
alphas=seq(0,1,by=.1); Rs=rep(0,length(alphas))
for(k in 1:length(alphas)){
  ...
  Rs[k]=eigen(diag(Nk,2,2) %*% beta)$values[1]
}
plot(alphas,Rs,xlab='alpha',ylab='R0',type='l',lwd=2)
########################
